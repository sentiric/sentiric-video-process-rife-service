...
class S3Uploader:
    ...
    def upload_file(self, local_path, job_id, trace_id):
        obj_name = f"refined/{job_id}.mp4"
        self.client.upload_file(local_path, settings.S3_BUCKET, obj_name)
        s3_uri = f"s3://{settings.S3_BUCKET}/{obj_name}"
        logger.info("Refined video uploaded to S3", event_id="S3_UPLOAD_SUCCESS", trace_id=trace_id, uri=s3_uri)
        return s3_uri
        ...
        try:
            ...
            async with conn:
                ...
                evt = event_pb2.MediaGenerationCompletedEvent(
                    ...
                    media_type="video_refined", success=success, result_uri=result_uri, 
                    error_message=error_msg, timestamp=ts
                )
                msg = aio_pika.Message(body=evt.SerializeToString(), content_type="application/protobuf")
                await ex.publish(msg, routing_key=event_type)
        except Exception as e:
            logger.error(f"RMQ Fail: {e}", event_id="RMQ_PUBLISH_FAIL")